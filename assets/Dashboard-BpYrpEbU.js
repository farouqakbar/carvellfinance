import{u as le,e as ce,d as de,r as g,s as d,j as e,L as D}from"./index-BdvH2DEd.js";import{g as W,a as E,f as o}from"./formatCurrency-CwSiFA8N.js";import{T as me}from"./TransactionForm-DsgIdoMX.js";import{C as pe}from"./CurrencyInput-DvdL0GVf.js";import{i as m}from"./mandatoryCategories-BLLVk51K.js";function he(c){const[b,f]=c.split("-").map(Number),h=new Date(b,f-2,1);return`${h.getFullYear()}-${String(h.getMonth()+1).padStart(2,"0")}`}function ge(c){const[b,f]=c.split("-").map(Number),h=new Date(b,f,1);return`${h.getFullYear()}-${String(h.getMonth()+1).padStart(2,"0")}`}function ye(){const{user:c}=le(),b=ce(),[f,h]=de(),[l,H]=g.useState(()=>f.get("month")||W()),[t,Y]=g.useState({salary:0,totalExpense:0,totalIncome:0,categories:[],transactions:[],savings:[],savingsLogs:[],categorySpend:[],todayExpense:0,totalTabungan:0}),[v,$]=g.useState(!0),[K,x]=g.useState(!1),[F,P]=g.useState(""),[U,u]=g.useState(!1);g.useEffect(()=>{S()},[l]);const q=a=>{H(a),h({month:a})},S=async()=>{var a,s;$(!0);try{const n=`${l}-01`,p=`${l}-31`,N=new Date().toISOString().split("T")[0],[y,ae,C,G,te,se]=await Promise.all([d.from("salaries").select("*").eq("user_id",c.id).eq("month",l).maybeSingle(),d.from("transactions").select("*, categories(name, color, icon)").eq("user_id",c.id).gte("date",n).lte("date",p).order("date",{ascending:!1}),d.from("categories").select("*").eq("user_id",c.id).order("name"),d.from("savings").select("*").eq("user_id",c.id),d.from("savings_log").select("*").eq("user_id",c.id).eq("month",l),d.from("transactions").select("amount").eq("user_id",c.id).eq("date",N).eq("type","expense")]),k=ae.data||[],re=C.data||[],ie=k.filter(r=>r.type==="expense").reduce((r,i)=>r+Number(i.amount),0),ne=k.filter(r=>r.type==="income").reduce((r,i)=>r+Number(i.amount),0),j={};k.filter(r=>r.type==="expense"&&r.categories).forEach(r=>{const i=r.categories.name;j[i]||(j[i]={name:i,amount:0,color:r.categories.color,icon:r.categories.icon}),j[i].amount+=Number(r.amount)});const oe=re.map(r=>{var z;const i=((z=j[r.name])==null?void 0:z.amount)||0,M=r.budget_limit>0?i/r.budget_limit*100:null;return{...r,spent:i,pct:M,overBudget:r.budget_limit>0&&i>r.budget_limit}}).sort((r,i)=>r.overBudget&&!i.overBudget?-1:!r.overBudget&&i.overBudget?1:(i.pct||0)-(r.pct||0));Y({salary:((a=y.data)==null?void 0:a.amount)||0,totalExpense:ie,totalIncome:ne,categories:oe,transactions:k.slice(0,5),savings:G.data||[],savingsLogs:te.data||[],todayExpense:(se.data||[]).reduce((r,i)=>r+Number(i.amount),0),totalTabungan:(G.data||[]).reduce((r,i)=>r+Number(i.current_amount),0),categorySpend:Object.values(j).sort((r,i)=>i.amount-r.amount)});const O=((s=y.data)==null?void 0:s.amount)||0;if(O>0){const r=(C.data||[]).filter(i=>m(i)&&!(Number(i.budget_limit)>0));if(r.length>0){const i=Math.round(Number(O)*.15);await Promise.all(r.map(z=>d.from("categories").update({budget_limit:i}).eq("id",z.id)));const{data:M}=await d.from("categories").select("*").eq("user_id",c.id).order("name");C.data=M}}}finally{$(!1)}},J=async()=>{const a=parseFloat(F);if(!a)return;await d.from("salaries").upsert({user_id:c.id,month:l,amount:a},{onConflict:"user_id,month"});const s=t.categories.filter(n=>m(n)&&!(Number(n.budget_limit)>0));if(s.length>0){const n=Math.round(a*.15);await Promise.all(s.map(p=>d.from("categories").update({budget_limit:n}).eq("id",p.id)))}b("Gaji disimpan","success"),x(!1),P(""),S()};t.categories.filter(a=>a.budget_limit>0).reduce((a,s)=>a+s.budget_limit,0);const Q=l===W(),R=t.categories.filter(a=>a.overBudget),V=.15,X=t.categories.filter(a=>m(a)).reduce((a,s)=>{const n=Number(s.budget_limit)>0?Number(s.budget_limit):t.salary>0?Math.round(t.salary*V):0;return a+n},0),Z=t.categories.filter(a=>m(a)).reduce((a,s)=>a+(s.spent||0),0),_=Math.max(0,X-Z),T=t.totalExpense+_,B=t.salary+t.totalIncome-T,w=t.salary>0?T/t.salary*100:0,A=w>90?"var(--danger)":w>70?"var(--warning)":"var(--accent)",I=t.categories.filter(a=>a.name==="Tabungan Bulanan"&&a.budget_limit>0).reduce((a,s)=>a+Number(s.budget_limit),0),L=t.salary>0?t.salary-I:0;L-t.totalExpense;const ee=t.salary>0&&I>0&&t.totalExpense>L;return e.jsxs("div",{className:"animate-in",children:[e.jsxs("div",{className:"dash-header",children:[e.jsxs("div",{className:"month-nav-group",children:[e.jsx("button",{className:"month-btn",onClick:()=>q(he(l)),children:"‹"}),e.jsx("span",{className:"month-label-text",children:E(l)}),e.jsx("button",{className:"month-btn",onClick:()=>q(ge(l)),disabled:Q,children:"›"})]}),e.jsxs("div",{className:"flex gap-8",children:[e.jsx("button",{className:"btn btn-secondary btn-sm",onClick:()=>x(!0),children:"Atur Gaji"}),e.jsx("button",{className:"btn btn-primary btn-sm",onClick:()=>u(!0),children:"+ Transaksi"})]})]}),R.length>0&&e.jsxs("div",{className:"alert-banner",children:[e.jsx("span",{children:"⚠"}),e.jsxs("span",{children:[e.jsx("strong",{children:"Overbudget"})," — ",R.map(a=>`${a.icon} ${a.name}`).join(", ")]})]}),e.jsxs("div",{style:{display:"flex",flexDirection:"column",gap:16},children:[e.jsx("div",{className:"hero-card",children:v?e.jsx("div",{className:"skeleton",style:{height:88,borderRadius:8}}):e.jsxs(e.Fragment,{children:[e.jsxs("div",{className:"hero-top",children:[e.jsxs("div",{className:"hero-left",children:[e.jsxs("span",{className:"hero-eyebrow",children:["Saldo Bersih ",E(l)]}),e.jsxs("div",{className:`hero-balance ${B<0?"neg":""}`,children:[B<0&&e.jsx("span",{className:"hero-neg-sign",children:"-"}),o(Math.abs(B))]})]}),e.jsxs("div",{className:"hero-right",children:[t.salary>0&&e.jsxs("div",{className:"hero-chip",children:[e.jsx("span",{className:"hero-chip-label",children:"Gaji"}),e.jsx("span",{className:"hero-chip-val tabular",children:o(t.salary)})]}),t.totalTabungan>0&&e.jsxs("div",{className:"hero-chip",children:[e.jsx("span",{className:"hero-chip-label",children:"Total Tabungan"}),e.jsx("span",{className:"hero-chip-val tabular",style:{color:"var(--success)"},children:o(t.totalTabungan)})]})]})]}),t.salary>0?e.jsxs("div",{className:"hero-bar-section",children:[e.jsx("div",{className:"hero-bar-track",children:e.jsx("div",{className:"hero-bar-fill",style:{width:`${Math.min(w,100)}%`,background:A}})}),e.jsxs("div",{className:"hero-bar-labels",children:[e.jsxs("span",{children:[o(t.totalExpense)," dipakai"]}),e.jsxs("span",{style:{color:A,fontWeight:700},children:[w.toFixed(0),"%"]})]})]}):e.jsxs("div",{className:"hero-no-salary",children:[e.jsx("button",{className:"salary-cta",onClick:()=>x(!0),children:"+ Atur gaji bulan ini"}),e.jsx("span",{className:"salary-cta-hint",children:"untuk menghitung saldo bersih"})]})]})}),e.jsxs("div",{className:"stats-strip",children:[e.jsxs("div",{className:"stat-col",children:[e.jsx("span",{className:"stat-col-label",children:"Pemasukan"}),e.jsxs("span",{className:"stat-col-val tabular",style:{color:"var(--success)"},children:["+",o(t.totalIncome)]})]}),e.jsxs("div",{className:"stat-col",children:[e.jsx("span",{className:"stat-col-label",children:"Pengeluaran"}),e.jsxs("span",{className:"stat-col-val tabular",style:{color:ee?"var(--danger)":"var(--text-primary)"},children:["-",o(T)]}),_>0&&e.jsxs("span",{className:"stat-col-sub",children:["+",o(_)," wajib"]})]}),e.jsxs("div",{className:"stat-col",children:[e.jsx("span",{className:"stat-col-label",children:"Belanja Hari Ini"}),e.jsx("span",{className:"stat-col-val tabular",style:{color:t.todayExpense>0?"var(--danger)":"var(--text-muted)"},children:t.todayExpense>0?`-${o(t.todayExpense)}`:"—"}),t.todayExpense>0&&t.salary>0&&e.jsxs("span",{className:"stat-col-sub",children:[(t.todayExpense/t.salary*100).toFixed(1),"% gaji"]})]})]}),e.jsxs("div",{className:"card",children:[e.jsxs("div",{className:"sect-head",children:[e.jsxs("div",{children:[e.jsx("h3",{className:"sect-title",children:"Pengeluaran Wajib"}),e.jsx("p",{className:"sect-sub",children:"Dipotong langsung dari gaji"})]}),e.jsx(D,{to:"/categories",className:"pill-link",children:"Kelola"})]}),v?e.jsx("div",{style:{display:"flex",flexDirection:"column",gap:8},children:[...Array(3)].map((a,s)=>e.jsx("div",{className:"skeleton",style:{height:36}},s))}):e.jsx("div",{className:"wajib-rows",children:t.categories.filter(a=>m(a)).map(a=>{const s=Number(a.budget_limit)>0?Number(a.budget_limit):t.salary>0?Math.round(t.salary*.15):0,n=t.salary>0&&s>0?Math.round(s/t.salary*100):null;return e.jsxs("div",{className:"wajib-row",children:[e.jsxs("div",{className:"wajib-left",children:[e.jsx("span",{className:"brow-icon",style:{background:`${a.color}18`,color:a.color},children:a.icon}),e.jsx("span",{className:"brow-name",children:a.name})]}),e.jsxs("div",{className:"wajib-right",children:[n&&e.jsxs("span",{className:"wajib-pct",children:[n,"%"]}),e.jsx("span",{className:"wajib-amount tabular",children:o(s)})]})]},a.id)})})]}),(v||t.categories.filter(a=>!m(a)&&a.budget_limit>0).length>0)&&e.jsxs("div",{className:"card",children:[e.jsx("div",{className:"sect-head",children:e.jsxs("div",{children:[e.jsx("h3",{className:"sect-title",children:"Budget Kategori"}),t.categories.filter(a=>!m(a)&&a.budget_limit>0).length>0&&e.jsxs("p",{className:"sect-sub",children:[o(t.categories.filter(a=>!m(a)).reduce((a,s)=>a+(s.spent||0),0))," dari ",o(t.categories.filter(a=>!m(a)&&a.budget_limit>0).reduce((a,s)=>a+Number(s.budget_limit),0))]})]})}),v?e.jsx("div",{style:{display:"flex",flexDirection:"column",gap:10},children:[...Array(2)].map((a,s)=>e.jsx("div",{className:"skeleton",style:{height:44}},s))}):e.jsx("div",{className:"budget-rows",children:t.categories.filter(a=>!m(a)&&a.budget_limit>0).map(a=>{const s=a.spent/a.budget_limit*100,n=Math.min(s,100),p=!a.overBudget&&s>=100,N=!a.overBudget&&s>=80&&s<100,y=a.overBudget?"var(--danger)":p?"var(--success)":N?"var(--warning)":a.color;return e.jsxs("div",{className:"brow",children:[e.jsxs("div",{className:"brow-left",children:[e.jsx("span",{className:"brow-icon",style:{background:`${a.color}18`,color:a.color},children:a.icon}),e.jsx("span",{className:"brow-name",children:a.name}),a.overBudget&&e.jsx("span",{className:"badge badge-danger",style:{fontSize:"0.6rem",padding:"2px 7px"},children:"Over"}),p&&e.jsx("span",{className:"badge badge-success",style:{fontSize:"0.6rem",padding:"2px 7px"},children:"Penuh"}),N&&e.jsx("span",{className:"badge badge-warning",style:{fontSize:"0.6rem",padding:"2px 7px"},children:"Hampir"})]}),e.jsx("div",{className:"brow-bar-wrap",children:e.jsx("div",{className:"brow-bar",children:e.jsx("div",{className:"brow-bar-fill",style:{width:`${n}%`,background:y}})})}),e.jsxs("div",{className:"brow-right",children:[e.jsx("span",{className:"brow-spent tabular",style:{color:a.overBudget?"var(--danger)":"var(--text-primary)"},children:o(a.spent)}),e.jsxs("span",{className:"brow-limit tabular",children:["/",o(a.budget_limit)]})]}),e.jsxs("span",{className:"brow-pct",style:{color:y},children:[n.toFixed(0),"%"]})]},a.id)})})]}),e.jsxs("div",{className:"card",style:{borderStyle:"dashed"},children:[e.jsxs("div",{className:"sect-head",children:[e.jsx("h3",{className:"sect-title",children:"Rencana Bulan Depan"}),e.jsx(D,{to:"/savings",className:"pill-link",children:"Atur →"})]}),e.jsx("p",{style:{fontSize:"0.78rem",color:"var(--text-muted)",margin:0},children:"Rencanakan pengeluaran bulan depan secara rinci di halaman Rencana."})]}),e.jsxs("div",{className:"card",children:[e.jsxs("div",{className:"sect-head",children:[e.jsx("h3",{className:"sect-title",children:"Transaksi Terakhir"}),e.jsx(D,{to:`/transactions?month=${l}`,className:"pill-link",children:"Lihat semua"})]}),v?e.jsx("div",{style:{display:"flex",flexDirection:"column",gap:8,marginTop:12},children:[...Array(4)].map((a,s)=>e.jsx("div",{className:"skeleton",style:{height:42}},s))}):t.transactions.length===0?e.jsxs("div",{className:"empty-hint",children:[e.jsx("span",{className:"empty-hint-icon",children:"↕"}),e.jsx("span",{children:"Belum ada transaksi bulan ini. "}),e.jsx("button",{className:"empty-hint-link",onClick:()=>u(!0),children:"Tambah sekarang →"})]}):e.jsx("div",{className:"tx-list",children:t.transactions.map(a=>{var s,n,p;return e.jsxs("div",{className:"tx-row",children:[e.jsx("div",{className:"tx-icon",style:{background:(s=a.categories)!=null&&s.color?`${a.categories.color}18`:"var(--bg-input)"},children:((n=a.categories)==null?void 0:n.icon)||(a.type==="income"?"↑":"↓")}),e.jsxs("div",{className:"tx-meta",children:[e.jsx("span",{className:"tx-desc",children:a.description||((p=a.categories)==null?void 0:p.name)||"Transaksi"}),e.jsx("span",{className:"tx-date",children:new Date(a.date).toLocaleDateString("id-ID",{day:"numeric",month:"short"})})]}),e.jsxs("span",{className:`tx-amount tabular ${a.type==="income"?"inc":"exp"}`,children:[a.type==="income"?"+":"−",o(a.amount)]})]},a.id)})})]})]}),K&&e.jsx("div",{className:"modal-overlay",onClick:()=>x(!1),children:e.jsxs("div",{className:"modal",style:{maxWidth:380},onClick:a=>a.stopPropagation(),children:[e.jsxs("div",{className:"modal-header",children:[e.jsxs("h2",{className:"modal-title",children:["Gaji ",E(l)]}),e.jsx("button",{className:"btn btn-ghost",onClick:()=>x(!1),children:"✕"})]}),e.jsxs("div",{className:"form-group",children:[e.jsx("label",{className:"form-label",children:"Gaji Bulan Ini"}),e.jsx(pe,{value:F,onChange:a=>P(a),autoFocus:!0})]}),e.jsxs("div",{className:"flex gap-8",children:[e.jsx("button",{className:"btn btn-secondary",onClick:()=>x(!1),children:"Batal"}),e.jsx("button",{className:"btn btn-primary",style:{flex:1},onClick:J,children:"Simpan"})]})]})}),U&&e.jsx("div",{className:"modal-overlay",onClick:()=>u(!1),children:e.jsxs("div",{className:"modal",onClick:a=>a.stopPropagation(),children:[e.jsxs("div",{className:"modal-header",children:[e.jsx("h2",{className:"modal-title",children:"Tambah Transaksi"}),e.jsx("button",{className:"btn btn-ghost",onClick:()=>u(!1),children:"✕"})]}),e.jsx(me,{onSuccess:()=>{S(),u(!1)},onClose:()=>u(!1)})]})}),e.jsx("style",{children:`
        /* ── Header ───────────────────────────── */
        .dash-header {
